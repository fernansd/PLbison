...
void init()
{
 ...
 for (i=0; constantes[i].nombre; i++)
 	{
 		v.num = constantes[i].cval;
 		install(constantes[i].nombre,CONSTANTE,NUMBER,v);
 	}
       

 /* Se pasa 0 para los install siguientes, parámetro innecesario */
 v.num = 0;

 for (i=0; keywords[i].nombre; i++)
 	{
       install(keywords[i].nombre,keywords[i].kval,INDEFINIDA,v);
 	}

 for (i=0; funciones0[i].nombre; i++)
     {
      s=install(funciones0[i].nombre,FUNCION0_PREDEFINIDA,INDEFINIDA,v);
      s->u.ptr=funciones0[i].func;
     }
 for (i=0; funciones1[i].nombre; i++)
     {
      s=install(funciones1[i].nombre,FUNCION1_PREDEFINIDA,INDEFINIDA,v);
      s->u.ptr=funciones1[i].func;
     }
 for (i=0; funciones2[i].nombre; i++)
     {
      s=install(funciones2[i].nombre,FUNCION2_PREDEFINIDA,INDEFINIDA,v);
